            ...
            private IEnumerable<ColumnInfo<PayrollReport.QueryResult.PayrollReportItem>> GetColumnsForPayrollReport(bool? deductedSSS, bool? deductedPagIbig, bool? deductedPHIC, bool? deductedTax, PayrollReport.QueryResult payrollReportResult, IEnumerable<PayrollReport.QueryResult.PayrollReportItem> payrollReportItems, bool? viewDetailed)
            {
                ...
                if (payrollReportItems.Select(p => p.PayrollRecord).Sum(p => p.LoanPaymentValue.GetValueOrDefault()) > 0)
                {
                    foreach (var loanType in payrollReportResult.LoanTypes)
                    {
                        if (payrollReportItems.SelectMany(p => p.Loans.Where(l => l.LoanTypeId == loanType.Id && l.RemainingBalance.GetValueOrDefault() > 0)).Sum(l => l.DeductionAmount.GetValueOrDefault()) > 0)
                        {
                            columns.Add(new ColumnInfo<PayrollReport.QueryResult.PayrollReportItem>($"{loanType.Code}", $"{loanType.Code}", p => String.Format("{0:n}", p.Loans.Where(l => l.LoanTypeId == loanType.Id && l.RemainingBalance.GetValueOrDefault() > 0).Sum(l => l.DeductionAmount.GetValueOrDefault()).ToString() ?? "0.00")));
                        }
                    }
                }
